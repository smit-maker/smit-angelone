...
function App() {
  ...
  const [currentPage, setCurrentPage] = useState<Page>(Page.Token);
  ...
  return (
    ...
    <div className="container border m-auto" style={{ width: 1800, height: 800}}>

      {loginToken ? (
        <div className="container border">

          <div className="nav nav-pills nav-fill gap-1 p-2 small bg-primary rounded-5 shadow-lg m-4" id="customNav" style={{ color: 'var(--bs-white)', backgroundColor: 'var(--bs-primary)', background: 'var(--bs-white)' }} role="tablist">
            ...
            <div className={`nav-item ${currentPage === Page.Token ? 'active bg-white rounded-5' : ''}`} role="presentation">
              <button className={`nav-link rounded-5 ${currentPage === Page.Token ? 'text-Black' : 'text-white'}`} onClick={() => setCurrentPage(Page.Token)}>Token</button> {/* aria-selected={currentPage === Page.About}*/}
            </div>
            
            {/* <div className="nav-item rounded-5" role="presentation"><button className="nav-link rounded-5"></button></div> */}
